...
import com.sakezuki.tools.translator.SakeTranslator;
    ...
    public static void main(String[] args){
        ...
        try(Connection conn=DriverManager.getConnection(
            ...
            TranslationRepository repository=
                    new TranslationRepository(conn);

            SakeTranslator translator=
                    new SakeTranslator(repository);

            translator.translateAll();

        }catch(Exception e){
            System.err.println(
                    "번역 실행 실패: "+e.getMessage()
            );
